...
6. 闲得慌也可以见[旧README](README_Linux.md)，~~废话更多~~，更详细。
...
3. **已知缺陷**：删除时只删了数据库里的记录，没有删除静态资源服务器里的文件，有强迫症的可以补个完整的删除；文档学习安卓上不太好内部实现，其他平台上建议可以实现下；学习列表和刷题列表也可以加一个检索；其实管理员登录成功后会返回一个用于验证的时间戳，每次管理员操作都要进行验证才能操作，但是开发、调试起来很麻烦就使用了，注重安全的可以用上并进行各种编码、混淆……~~虽然作为毕设应该也没人会在意。~~
